...
namespace FoodReserve.OutletPortal.Services
{
    public class QueueService(ProtectedSessionStorage sessionStorage, IHttpClientFactory httpClientFactory)
    {
        ...
        public async Task<PaginatedResponse<IEnumerable<QueueResponse>>> GetAllAsync(int pageNumber, int pageSize, string? keyword)
        {
            var response = await _httpClient.GetAsync($"api/outlet/queue?pageNumber={pageNumber}&pageSize={pageSize}&keyword={keyword}");
